...
class MyAgent(Agent):
    ...
    async def on_enter(self):
        ...
        try:
            if has_history:
                # Generate reconnection greeting using LLM directly
                logger.info("🔄 Generating spontaneous reconnection greeting")
                reconnect_system_prompt = """Je bent AVA. De gebruiker komt terug na een eerdere sessie.
Begroet de gebruiker kort en warm, en nodig uit om verder te praten.

Houd het kort (2 zinnen max). Vraag wat de gebruiker wil bespreken."""
                
                reconnect_ctx = ChatContext.empty()
                reconnect_ctx.add_message(role="system", content=reconnect_system_prompt)
                reconnect_ctx.add_message(role="user", content="[gebruiker is opnieuw verbonden]")
                
                greeting_stream = fallback_llm.chat(chat_ctx=reconnect_ctx)
                greeting_parts = []
                async for chunk in greeting_stream:
                    if hasattr(chunk, 'delta') and hasattr(chunk.delta, 'content'):
                        if chunk.delta.content:
                            greeting_parts.append(chunk.delta.content)
                    elif hasattr(chunk, 'choices') and chunk.choices:
                        delta = chunk.choices[0].delta
                ...
                if not greeting:
                    greeting = "Welkom terug! Fijn dat je er weer bent."
            else:
                ...
                if not greeting:
                    greeting = "Hallo, welkom. Wat brengt je hier vandaag?"
